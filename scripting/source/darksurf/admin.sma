...
public plugin_init() {
	register_plugin("DARKSURF - ADMIN","T9k","Team9000")

	register_concmd("amx_surf_points","AdminPoints",LVL_DARKSURF,"<authid, nick, @team or #userid> [points]")
	register_concmd("amx_surf_skill","AdminSkill",LVL_DARKSURF,"<authid, nick, @team or #userid> <skill> [level]")
}

public plugin_natives() {
	register_library("darksurf_admin")
}
...
public AdminPoints(id,level,cid) { 
	...
	if(read_argc() < 3) {
		...
		if(cmd_targetset(id, target, 0, targetname, 32)) {
			while((targetindex = cmd_target())) {
				...
				if(!surf_playerloaded(targetindex)) {
					console_print(id,"%s is not loaded", name)
					continue
				}

				console_print(id,"%s has %d Points", name, surf_getpoints(targetindex))
			}
		}
		return PLUGIN_HANDLED
	}
	...
	if(cmd_targetset(id, target, 3, targetname, 32)) {
		while((targetindex = cmd_target())) {
			...
			surf_setpoints(targetindex, ipoints)
...
public AdminSkill(id,level,cid) { 
	...
	if(read_argc() < 4) {
		...
		if(cmd_targetset(id, target, 0, targetname, 32)) {
			while((targetindex = cmd_target())) {
				...
				new skillname[32]
				surf_get_skillname(iskill, skillname, 31)
				new skillshort[32]
				surf_get_skillshort(iskill, skillshort, 31)
				console_print(id,"%s has a %s level of %d", name, skillname, surf_getskill(targetindex, skillshort))
			}
		}
		return PLUGIN_HANDLED
	}
	...
	if(cmd_targetset(id, target, 3, targetname, 32)) {
		while((targetindex = cmd_target())) {
			...
			surf_setskill(targetindex, skillshort, ilevel)
			storage_saveplayer(targetindex)
